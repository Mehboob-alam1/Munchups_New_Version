...
class EditGrocerPostItemFormPage extends StatefulWidget {
  ...
  final String serviceType; // "Pickup" or "Drop"
  final String? image1;
  final String? image2;
  final String? image3;

  const EditGrocerPostItemFormPage({
    super.key,
...
class _EditGrocerPostItemFormPageState
  ...
  void initState() {
    ...
    pickAndDrop = widget.serviceType;
    networkImage1 = widget.image1;
    networkImage2 = widget.image2;
    networkImage3 = widget.image3;
  }
  ...
  Widget itemNameField() {
    return InputFieldsWithLightWhiteColor(
      ...
      style: black15bold,
      validator: (val) {
        if (val.isEmpty) return TextStrings.textKey['field_req']!;
        return null;
      },
      onChanged: (value) {
        setState(() {
    ...
    return CommanButton(
      ...
      onPressed: () {
        if (globalKey.currentState!.validate()) {
          // Form is valid — handle in parent or controller
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('Form is valid. Submit logic goes elsewhere.')),
          );
        }
      },
    );
