...
class EditChefPostDishForm extends StatefulWidget {
  ...
  const EditChefPostDishForm({super.key, required this.dishData});
...
class _EditChefPostDishFormState extends State<EditChefPostDishForm> {
  ...
  List portionPerPersionList = List.generate(10, (index) => '${index + 1}');
  List deliveryRangList = List.generate(50, (index) => '${index + 1}');
  ...
  void initState() {
    ...
    dishName = dishData['dish_name'] ?? '';
    selectMeal = dishData['meal'] ?? 'Breakfast';
    collecAndDelivery = dishData['service_type'] ?? 'Collection';
    dishSrevingDate = dishData['dish_date'] ?? '';
    dishServingDate.text = dishData['dish_date'] ?? '';
    dishServingTimeFrom.text = dishData['start_time'] ?? '';
    dishServingTimeTo.text = dishData['end_time'] ?? '';
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: PreferredSize(
        preferredSize: const Size.fromHeight(60),
        child: BackIconCustomAppBar(title: TextStrings.textKey['post_dish']!),
      ),
      body: SingleChildScrollView(
        child: Form(
          key: globalKey,
          child: Padding(
            ...
            child: Column(
              children: [
                SizedBox(height: SizeConfig.getSize30(context: context)),
                 CommanImagePicker(networkImage1: '', networkImage2: '', networkImage3: '',),
                SizedBox(height: SizeConfig.getSize20(context: context)),
                buildDishNameField(),
                SizedBox(height: SizeConfig.getSize10(context: context)),
                CommanDropDown(
                  ...
                  list: mealList,
                  selectedData: selectMeal,
                  onChanged: (value) => setState(() => selectMeal = value),
                ),
                SizedBox(height: SizeConfig.getSize10(context: context)),
                buildCollectionAndDeliveryRadio(),
                SizedBox(height: SizeConfig.getSize10(context: context)),
                buildDatePicker(),
                SizedBox(height: SizeConfig.getSize10(context: context)),
                Row(
                  children: [
                    Expanded(child: buildTimePickerFrom()),
                    SizedBox(width: SizeConfig.getSize10(context: context)),
                    Expanded(child: buildTimePickerTo()),
                  ],
                ),
                SizedBox(height: SizeConfig.getSize10(context: context)),
                SingleFoodCategoryDropDown(
                ...
                CommanDropDown(
                  ...
                  list: portionPerPersionList,
                  selectedData: selectedPerPersion,
                  onChanged: (value) => setState(() => selectedPerPersion = value),
                ),
                SizedBox(height: SizeConfig.getSize10(context: context)),
                buildPriceField(),
                SizedBox(height: SizeConfig.getSize10(context: context)),
                CommanDropDown(
                  ...
                  list: deliveryRangList,
                  selectedData: selectedDiliveryRange,
                  onChanged: (value) => setState(() => selectedDiliveryRange = value),
                ),
                SizedBox(height: SizeConfig.getSize10(context: context)),
                    ...
                    if (globalKey.currentState!.validate()) {
                      showDialog(
                        ...
                        builder: (_) => AlertDialog(
                          ...
                          content: const Text(
                              'This is a frontend-only form.\nNo backend action is performed.'),
                          actions: [
                            TextButton(
  ...
  Widget buildDishNameField() => InputFieldsWithLightWhiteColor(
    initialValue: dishName,
    labelText: 'Dish Name',
    validator: (val) => val.isEmpty ? 'Required field' : null,
    onChanged: (value) => setState(() => dishName = value),
  );

  Widget buildCollectionAndDeliveryRadio() => Row(
    mainAxisAlignment: MainAxisAlignment.center,
    children: [
      Radio(
        value: 'Collection',
        groupValue: collecAndDelivery,
        onChanged: (value) => setState(() => collecAndDelivery = value!),
      ),
  ...
  Widget buildDatePicker() => InputFieldsWithLightWhiteColor(
    controller: dishServingDate,
    readOnly: true,
    labelText: 'Dish Serving Date',
    onTap: () async {
  ...
  Widget buildTimePickerFrom() => InputFieldsWithLightWhiteColor(
    controller: dishServingTimeFrom,
    readOnly: true,
    labelText: 'Time From',
    onTap: () async {
  ...
  Widget buildTimePickerTo() => InputFieldsWithLightWhiteColor(
    controller: dishServingTimeTo,
    readOnly: true,
    labelText: 'Time To',
    onTap: () async {
  ...
  Widget buildPriceField() => InputFieldsWithLightWhiteColor(
    initialValue: price,
    labelText: 'Dish Price',
    keyboardType: TextInputType.number,
    validator: (val) => val.isEmpty ? 'Required field' : null,
  ...
  Widget buildDescriptionField() => InputFieldsWithLightWhiteColor(
    initialValue: description,
    labelText: 'Dish Description',
    maxLines: 5,
