...
import '../../../Screens/Buyer/Card Payment/Credit Card Formats/card_number_formats.dart';
import '../../../Comman widgets/app_bar/back_icon_appbar.dart';
...
class _CardPaymentFormPageState extends State<CardPaymentFormPage> {
            ...
            Center(
              child: ElevatedButton(
                ...
                onPressed: isFormValid()
                  ...
                  // This is where you would normally call a Stripe/payment function
                  ScaffoldMessenger.of(context).showSnackBar(
                    const SnackBar(content: Text('Card Submitted')),
                  );
                }
                    : null,
                child: const Padding(
  ...
  Future<void> selectMonth(BuildContext context) async {
    final int? pickedMonth = await showDialog<int>(
      context: context,
      builder: (_) => MonthPickerDialog(initialMonth: expiryMonth),
    );

    if (pickedMonth != null) {
      setState(() {
        expiryMonth = pickedMonth;
        expiryMonthController.text =
        pickedMonth < 10 ? '0$pickedMonth' : '$pickedMonth';
      });
    }
  }
  ...
  Future<void> selectYear(BuildContext context) async {
    final int? pickedYear = await showDialog<int>(
      context: context,
      builder: (_) => YearPickerDialog(initialYear: expiryYear),
    );

    if (pickedYear != null) {
      setState(() {
        expiryYear = pickedYear;
        expiryYearController.text = '$pickedYear';
      });
    }
  }
}
...
class MonthPickerDialog extends StatelessWidget {
  ...
  const MonthPickerDialog({required this.initialMonth});
      ...
      content: SizedBox(
        ...
        child: Wrap(
          children: months.map((month) {
            final bool isSelected = month['value'] == initialMonth;
            return Padding(
              padding: const EdgeInsets.all(10.0),
              child: InkWell(
                ...
                child: Container(
                  ...
                  child: Text(month['name'], style: white14w5),
                ),
              ),
            );
          }).toList(),
...
class YearPickerDialog extends StatelessWidget {
  ...
  const YearPickerDialog({required this.initialYear});
    ...
    return AlertDialog(
      ...
      content: SizedBox(
        ...
        child: YearPicker(
          ...
          currentDate: DateTime(initialYear),
          selectedDate: DateTime(initialYear),
          onChanged: (dateTime) => Navigator.pop(context, dateTime.year),
        ),
      ),
    );
  }
