...
class OrderDetailPage extends StatelessWidget {
  ...
  const OrderDetailPage({super.key, required this.userType, required data});
  ...
  Widget build(BuildContext context) {
    ...
    final Map<String, dynamic> data = {
      'order_unique_number': 'ORD#001234',
      'order_date': '2025-08-01',
      'delivery_status': 'delivered',
      'status': 'accept',
      'address': '123 Flutter Street, Widget City',
      'user_type': 'chef',
      'chef_grocer_id': '99',
      'order_id': '56789',
      'item': [
    ...
    double totalAmount = data['item']
        .fold(0.0, (sum, item) => sum + double.parse(item['total_amount']));
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            orderHeader(data),
            const SizedBox(height: 20),
            deliveryDetails(data, userType),
            const Divider(color: DynamicColor.white),
            const SizedBox(height: 10),
            Text('ITEM', style: lightWhite15Bold),
            const SizedBox(height: 15),
            ...data['item'].map<Widget>((item) => itemTile(context, item)).toList(),
            const SizedBox(height: 20),
            const Divider(color: DynamicColor.white),
            totalCard(data['item'].length, totalAmount),
          ],
        ),
      ),
    );
  ...
  Widget orderHeader(Map<String, dynamic> data) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Row(
          mainAxisAlignment: MainAxisAlignment.spaceBetween,
          children: [
            Text('Order ID: ${data['order_unique_number']}', style: white17Bold),
            Row(
              children: [
                Text('Status: ', style: lightWhite15Bold),
                Text(
        ...
        Text(data['order_date'], style: white15bold),
      ],
    );
  }

  Widget deliveryDetails(Map<String, dynamic> data, String userType) {
    return Row(
      mainAxisAlignment: MainAxisAlignment.spaceBetween,
      children: [
        Expanded(
          ...
          child: Column(
            ...
            children: [
              Text('Delivered to', style: lightWhite14Bold),
              Text(data['address'], style: white15bold, maxLines: 1, overflow: TextOverflow.ellipsis),
            ],
          ),
        ),
        Expanded(
          child: userType != 'Buyer'
            ...
            children: [
              InkWell(
                ...
                child: actionButton('Rate'),
              ),
              const SizedBox(height: 10),
              InkWell(
                onTap: () {
                  PageNavigateScreen().push(context!, ReportPage(data: data));
                },
                child: actionButton('Report'),
              ),
            ],
          ),
        )
  ...
  Widget itemTile(BuildContext context, Map<String, dynamic> item) {
    return ListTile(
      contentPadding: const EdgeInsets.only(bottom: 8),
      leading: ClipRRect(
        borderRadius: BorderRadius.circular(8),
        child: InkWell(
          onTap: () {
            PageNavigateScreen().push(
              context,
              FullImageView(url: item['dish_images'][0]['kitchen_image']),
            );
          },
          child: CustomNetworkImage(url: item['dish_images'][0]['kitchen_image']),
        ),
      ),
      title: Text(item['name'], style: white17Bold, overflow: TextOverflow.ellipsis),
      subtitle: Text('Quantity: ${item['quantity']}', style: white14w5),
      trailing: Text('\$${item['dish_price']}', style: greenColor15bold),
    );
  }

  Widget totalCard(int itemCount, double totalAmount) {
    return Card(
      color: DynamicColor.boxColor,
      shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(10)),
      child: Padding(
        ...
        child: Column(
          children: [
            rowItem('Item:', itemCount.toString()),
            const SizedBox(height: 10),
            rowItem('Sub Total:', '\$${totalAmount.toStringAsFixed(2)}'),
            const Divider(color: DynamicColor.white),
            rowItem('Total', '\$${totalAmount.toStringAsFixed(2)}', isBold: true),
          ],
        ),
      ),
    );
  ...
  Widget rowItem(String label, String value, {bool isBold = false}) {
    return Row(
      mainAxisAlignment: MainAxisAlignment.spaceBetween,
      children: [
        Text(label, style: isBold ? white21bold : white15bold),
        Text(value, style: isBold ? primary25bold : white15bold),
      ],
    );
  }
  ...
  Widget actionButton(String text) {
    return Container(
      ...
      child: Text(text, style: white13w5),
    );
  }
}
