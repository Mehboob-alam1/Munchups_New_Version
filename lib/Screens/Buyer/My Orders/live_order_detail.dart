...
class _LiveOrderDetailPageState extends State<LiveOrderDetailPage> {
    ...
    return Row(
      ...
      children: [
        Column(
          children: [
            Text('Order ID', style: white17Bold),
            Text(data['order_unique_number'].toString(), style: white17Bold),
          ],
        ),
        Column(
          children: [
  ...
  Widget _actionButtons() {
    if (data['order_status'] == 'complete') {
      return CommanButton(
        ...
        width: MediaQuery.of(context).size.width * 0.5,
        buttonName: 'Download Invoice',
        onPressed: () => downloadInvoiceApiCall(context),
      );
    }
        ...
        widget.fromuserName == 'Chef'
          ...
          onPressed: () {
            PageNavigateScreen().push(
              context,
              ChefReportFormPage(
                orderUniqueNumber: data['order_unique_number'].toString(),
              ),
            );
          },
        )
  ...
  Widget orderDetail() {
    return _sectionCard(
      title: 'Order Detail',
      children: [
        customWidget(title: 'Occassion', value: data['occasion_category_name']),
        customWidget(title: 'Number Of People', value: data['no_of_people']),
        customWidget(title: 'Food Category', value: data['food_cate_name']),
        customWidget(title: 'Service Type', value: data['service_type']),
        customWidget(title: 'Budget', value: '$currencySymbol${data['budget']}'),
        customWidget(
          title: 'Date',
          value: '${data['start_date']} - ${data['end_date']}',
        ),
  ...
  Widget chefDetail() {
    return _sectionCard(
      title: '${widget.fromuserName} Detail',
      children: [
        customWidget(
          title: '${widget.fromuserName} Name',
          value: '${data['first_name']} ${data['last_name']}',
        ...
        customWidget(
          title: '${widget.fromuserName} Email',
          value: data['email_id'] ?? 'Not available',
        ),
        customWidget(
          title: '${widget.fromuserName} Address',
          value: formatAddress(data['location']),
        ...
        child: Column(
          children: [
            Padding(
              ...
              child: Text(title, style: primary17w6),
            ),
            ...children,
          ],
        ),
  ...
  void downloadInvoiceApiCall(BuildContext context) async {
    Utils().showSpinner(context);
    try {
      final response = await GetApiServer().downloadInvoiceApi(data['order_unique_number']);
      Utils().stopSpinner(context);
      if (response['success'] == 'true') {
        Utils.launchUrls(response['url'], context);
        PageNavigateScreen().pushRemovUntil(context, const ChefHomePage());
      }
    } catch (e) {
      log(e.toString());
