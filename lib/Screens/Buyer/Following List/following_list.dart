...
class _FollowingListState extends State<FollowingList> {
  ...
  // Mock data
  List<Map<String, String>> mockFollowers = [
    {
      'firstName': 'Emily',
      'lastName': 'Johnson',
      'userType': 'chef',
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: PreferredSize(
        preferredSize: const Size.fromHeight(60),
        child: BackIconCustomAppBar(title: 'Following'),
      ),
      body: mockFollowers.isEmpty
          ? const Center(child: Text('No Following available'))
          : Padding(
        padding: const EdgeInsets.all(8.0),
        child: ListView.builder(
          itemCount: mockFollowers.length,
          itemBuilder: (context, index) {
            final data = mockFollowers[index];
              ...
              child: ListTile(
                onTap: () {
                  ScaffoldMessenger.of(context).showSnackBar(
                    SnackBar(
                        content: Text(
                            'Tapped on ${data['firstName']}\'s profile')),
                  );
                },
                minLeadingWidth: 0.0,
                horizontalTitleGap: 10,
