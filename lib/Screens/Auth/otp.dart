...
class OtpPage extends StatefulWidget {
  ...
  const OtpPage({Key? key, required this.emailId, required this.type}) : super(key: key);
...
class _OtpPageState extends State<OtpPage> {
        ...
        child: SingleChildScrollView(
          child: Column(
            children: [
              const SizedBox(height: 60),
              backButton(),
              Image.asset(appLogoUrl, height: 150),
              const SizedBox(height: 40),
              instructionText(),
              const SizedBox(height: 50),
              otpInput(),
  ...
  Widget backButton() {
    return Align(
      ...
      child: IconButton(
        padding: const EdgeInsets.only(left: 15),
        icon: const Icon(Icons.arrow_back, size: 30, color: DynamicColor.white),
    ...
    return ListTile(
      ...
      subtitle: Column(
        children: [
          ...
          Text('We have sent the verification code', style: white17Bold),
          Text('to your email address', style: white17Bold),
        ],
      ),
    );
  ...
  Widget resendOtp() {
    return Align(
      ...
      child: Padding(
        ...
        child: TextButton(
          onPressed: () {
            // Just show a snackbar in frontend version
            ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(content: Text('OTP Resent (simulated)')),
            );
          },
          child: Text('Resend OTP', style: white17Bold),
        ),
      ),
    );
  }
          ...
          child: Column(
            children: [
              const Text(
                ...
                style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: DynamicColor.green),
                textAlign: TextAlign.center,
              ),
              const SizedBox(height: 25),
              CommanButton(
